...
public class PingPongMessageTest {
	...
	public void testDeserialization() {
		PingPongMessage actualMessage = (PingPongMessage) ConverterUtils.deserialize(createSerializedMessage(type,
				command, senderId,
				echoedRandomId, randomId));
		PingPongMessage expectedMessage = new PingPongMessage(type, command, senderId,
				echoedRandomId, randomId);
	...
	public void testSerDeserMessage() {
		PingPongMessage initialMessage = new PingPongMessage(type, command, senderId, echoedRandomId, randomId);
		ByteBuffer serializedMessage = ConverterUtils.serialize(initialMessage);
		PingPongMessage deserializedMessage = (PingPongMessage) ConverterUtils.deserialize(serializedMessage);
