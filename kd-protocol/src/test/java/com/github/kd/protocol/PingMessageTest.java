...
public class PingMessageTest {
	...
	public void testDeserialization() {
		PingMessage actualMessage = (PingMessage) ConverterUtils.deserialize(createSerializedMessage(
				type, command, senderId,
				randomId));
		PingMessage expectedMessage = new PingMessage(type, command, senderId, randomId);
	...
	public void testSerDeserMessage() {
		PingMessage initialMessage = new PingMessage(type, command, senderId, randomId);
		ByteBuffer serializedMessage = ConverterUtils.serialize(initialMessage);
		PingMessage deserializedMessage = (PingMessage) ConverterUtils.deserialize(serializedMessage);
