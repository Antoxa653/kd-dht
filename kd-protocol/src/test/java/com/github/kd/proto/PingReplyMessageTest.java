...
import com.github.kd.protocol.converter.PingReplyMessageConverter;

public class PingReplyMessageTest {
	private PingReplyMessageConverter converter = new PingReplyMessageConverter();

	private MessageType type = MessageType.REPLY;

	private MessageCommand command = MessageCommand.PING_PONG;
	...
	public void testDeserialization() {
		PingReplyMessage actualMessage = converter.deserialize(createSerializedMessage(type, command, senderId,
				echoedRandomId));
		PingReplyMessage expectedMessage = new PingReplyMessage(type, command, senderId, echoedRandomId);
	...
	public void testSerDeserMessage() {
		PingReplyMessage initialMessage = new PingReplyMessage(type, command, senderId, echoedRandomId);
		ByteBuffer serializedMessage = converter.serialize(initialMessage);
		PingReplyMessage deserializedMessage = converter.deserialize(serializedMessage);
